...
;cows are not called bison, +1 point for answering false -1 for answering true
jb p1t,p1_q8_lose
...
;measuring from P0.0
    ...
    setb TR2
    ;this branch is not used in this code
    period_loop:
    jb P0.0, $
    jnb P0.0, $
    ;djnz R0, period_loop
...
measure2:    
    ...
    ;as in the example  [overflow_counter+1,overflow_counter+0,TH2,TL2] * 45211.23ps is the period
	...
	Load_y(45211) ; One clock pulse is 45211.23ps
	lcall mul32
	;now x stores #_of_pulses*clock_period(ps) to get period or picoseconds transpired 
	...
	Load_y(2036)
	lcall div32 ;now x has capacitence in pf 
...
measure23:    
	...
	Load_y(45211) ; One clock pulse is 45211.23ps, the 
	lcall mul32
	;now x stores #_of_pulses*clock_period(ps) to get period or picoseconds transpired 
	...
	clr mf ;button not pressed by default
	Load_y(11000) ;if C>11000pf then the button was pressed
...
measure24:    
	...
	Load_y(45211) ; One clock pulse is 45211.23ps, the s
	...
	;make sure to jump to qunum_updated so that we return to the same question
	ljmp qnum_updated
